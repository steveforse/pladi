...
export function ImageModal({ movie, imageUrl, hasPrev, hasNext, position, total, onClose, onPrev, onNext }: ImageModalProps) {
  useEffect(() => {
  ...
  return createPortal(
    ...
    >
      <div className="flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
        <button className={goldBtn} onClick={onPrev} disabled={!hasPrev} aria-label="Previous">
          <ChevronLeft size={40} />
        </button>
        ...
        <button className={goldBtn} onClick={onNext} disabled={!hasNext} aria-label="Next">
