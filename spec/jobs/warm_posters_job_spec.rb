...
RSpec.describe WarmPostersJob do
  ...
  it 'broadcasts for movie ids with poster data' do
    expect(cable_server).to have_received(:broadcast).with("posters_#{server.id}", { movie_id: '1' })
  end

  it 'does not broadcast when poster is unavailable' do
    expect(cable_server).not_to have_received(:broadcast).with("posters_#{server.id}", { movie_id: '2' })
  end
end
